...
def serialize_etudiant(etudiant, modules):
    return {
        "id": etudiant.id,
...
def list_etudiants(request):
    ...
    if user.role == User.Role.ADMIN:
        ...
        etudiants = Etudiant.objects.select_related("user", "filiere").prefetch_related("modules")
        ...
        cours = Cours.objects.filter(enseignant=enseignant, actif=True).select_related("module", "filiere")
        accessible_filieres = Filiere.objects.filter(cours__in=cours).distinct()
        ...
        etudiants = (
            ...
            .filter(filiere__in=accessible_filieres, modules__in=accessible_modules)
            .distinct()
        )
    else:
        return Response(
    ...
    if semester:
        etudiants = etudiants.filter(modules__semestre=semester).distinct()
    ...
    for etudiant in etudiants:
        modules = [module for module in etudiant.modules.all() if module.id in accessible_module_ids]
        serialized_etudiants.append(serialize_etudiant(etudiant, modules))
